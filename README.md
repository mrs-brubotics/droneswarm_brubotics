...
* Horizon Europe euROBIN
* Horizon Europe SPEAR
* Innoviris Spectre

All packages are extensions and alternatives to those provided by the [Multi-robot Systems (MRS) Group of the CTU in Prague](https://github.com/ctu-mrs) and are organized in the same way. We mostly work with multi-rotor aerial vehicles, but these methods are also tested on robotic manipulators for safe human-robot coexistence. This framework can be tested both in simulation and on real-world robotic platforms. Real-world and replicable experiments should support excellent research and science in robotics. This platform is built to allow certified safe verification of these robot autonomy methods across planning, control, estimation, computer vision, tracking, and more.
...
* f450: used for swarming
* t650: can also be used for (collaborative) load transport

## Related packages
<!-- TODO[add those similar as ctu]-->
The following packages are not necessarily part of our automatic installation. Therefore, you might need to clone some of them by yourself and place them in your ROS workspace. Some of those are forks of third-party repositories.
...
### Native installation
#### Prerequisites
* Install Ubuntu 20.04 LTS desktop on a sufficiently powerful machine intended to be used as a simulation desktop (most demanding due to Gazebo simulation) or as on-board UAV computer. Note that most cheaper laptops give poor performance when running this framework. Follow [these instructions](https://ubuntu.com/tutorials/install-ubuntu-desktop#1-overview), boot from USB flash drive by creating a bootable memory stick as explained in [this tutorial](https://ubuntu.com/tutorials/create-a-usb-stick-on-ubuntu#1-overview). Download the Ubutnu .iso desktop image [here](https://releases.ubuntu.com/focal/).
Note <!-- TODO[move to tutorial chapter explaining the ubuntu settings]:--> (re)installing Ubuntu on the lab desktop should be done with caution. Please contact the responsible for this package in case Ubuntu needs to be reinstalled on that machine.
* We recommend configuring the Ubuntu machine as explained in [this tutorial](https://github.com/mrs-brubotics/documentation_brubotics/blob/main/source/Information_for_Internal_Collaborators.rst) to use all its functionalities.
* We assume the user regularly updates and upgrades the packages on the Ubuntu machine to the most recent version by running the Software Updater or by:
     ...
     ```
     If there are packages that fail to be updated or upgraded, first resolve this issue.
...
* Refer yourself to the [mrs_uav_system](https://github.com/ctu-mrs/mrs_uav_system#native-installation) and their [tutorial](https://ctu-mrs.github.io/) to natively install and build the software. We do not require you to install [ctu's Linux environment](https://github.com/ctu-mrs/mrs_uav_system#i-want-the-linux-environment-people-from-mrs-works-with). Before installation, check [here](https://github.com/ctu-mrs/mrs_uav_system#meta-repositories) that the build status of the mrs_uav_system is passing (green). If the build status is failing (red), you can copy their default install commands but replace the line
     ...
     ```
     which downgrades the mrs_uav_system to ensure compatibility with the droneswarm_brubotics system.
...
* If too much RAM is required during the building process your screen will freeze, try ```catkin build -j2``` which ensures not more than two processes are used in parallel for building the software. Repeat ```catkin build``` until there are no errors and no warnings any more.
...
* Avoid changing any code in any of CTU's packages, unless there is no other way to implement your functionality. If your application requires custom settings in CTU's packages, document clearly where these changes are required since these will need to be (un)done manually each time the software is reinstalled or used for other purposes.
* Regularly reinstall ctu's mrs_uav_system as it is evolving continuously and update the commits of all packages for which the droneswarm_brubotics codes work in this readme.
* If after installation of the mrs_uav_system, you cannot update your machine anymore as described above and when shutting down, the Ubuntu loader displays forever, there might be this known and solved issue with the google-guest-agent service. Follow the steps [here](https://github.com/ctu-mrs/mrs_uav_system/discussions/76) and use htop to kill the google-guest processes. After these steps make sure you can update your system. If you cannot, go to Ubutnu's Software Updater and follow the steps.
...
These packages are required dependencies of droneswarm_brubotics which have to be installed to obtain full functionality:
...
```
* Follow the output generated during the installation process. There are no issues if during the 'Installing dependencies...' all repos are cloned and checked out successfully  (e.g., no uncommitted changes). You should now see these cloned repos in the git folder and in the workspace folder.
* Installing the first time on a new machine will throw the error: ```git@github.com: Permission denied (public key)```. 
  * First, you need to set up your ssh keys correctly by following [these steps](https://docs.github.com/en/github/authenticating-to-github/generating-a-new-ssh-key-and-adding-it-to-the-ssh-agent). This needs to be done for each machine you use. The current ssh keys can be found in ```home/.shh```. Make sure you enables "Show Hidden Files" in the ```Home``` directory. 
  * Since August 2021 developers have been required to use [personnel access tokens](https://github.blog/2020-12-15-token-authentication-requirements-for-git-operations/). Follow [these steps](https://docs.github.com/en/github/authenticating-to-github/keeping-your-account-and-data-secure/creating-a-personal-access-token) to generate these tokens.
...
* Open ```~/mrs_workspace/src/simulation/ros_packages/mrs_simulation/models/mrs_robots_description/urdf/f450.xacro``` and adjust the mass: ```<xacro:property name="mass" value="${2.40-0.005*4.0-0.015-0.00001}" /> <!-- [kg] 2.40--> ```. This ensures that Gazebo simulates a UAV model with the hardware mass. Note that the xacro has a slight offset from 2.4kg since afterward some small masses (of motors, sensors) are added to the UAV so we subtract them before they are added.
* Open ```~/mrs_workspace/src/uav_core/ros_packages/mrs_uav_managers/config/simulation/f450/mass.yaml``` and adjust the mass: ```uav_mass: 2.40 #2.00 # [kg]```. This ensures that the controllers and trackers that use mass (e.g., for feedforward actions) use the hardware mass.
* Catkin build the mrs_worspace (although not strictly necessary if you only change configs, make a habit of catkin build more than too less)
* Do not forget to do the above steps each time you reinstall the mrs_uav_system!
For hardware experiments the UAV mass used in the controllers and trackers is the one set in the ~/.bashrc, hence the above changes do not affect operation on hardware.
* For UAVs with payload, you need to do the same for what concerns the mass of only the UAV (excluding payload mass), but you also need to ensure that the xacro of the payload has the same payload mass as the one you use in the controller and tracker. For 2 UAVs each UAV of course compensates for half of the bar's mass instead of the total payload mass in the case of one UAV with a cable-suspended load.
__Motor parameters__:
In order to simulate and experiment with correct motor parameters that resemble hardware operation please take a look at [regulation_control_predictions_one_drone_rtk](https://github.com/mrs-brubotics/testing_brubotics/tree/master/tmux_scripts/bryan/regulation_control_predictions_one_drone_rtk) for what concerns the ```thrust_saturation``` value in ```custom_configs/gains``` and the ```motor_params``` in custom ```configs/motor_params_hardware.yaml``` and in ```custom configs/motor_params_simulation.yaml```. These are configured for the f450 UAV, the values for the t650 UAV are TODO.
...
If you would like to reinstall everything just delete the git, the mrs_workspace, and the workspace folder.
This procedure is the same for each installed package. 
If there are installation or building problems that you cannot solve, please open an issue immediately.
...
* For data processing and plotting Matlab and Simulink version 2021a is at least required with the following toolboxes installed:
  ...
  * TODO add all toolboxes 
  ...
  ``` 
  but it will not necessarily install the newest CMake version of the code block. When you have not deleted the old version of CMake you won't automatically find a new version and an error is reported in the terminal when checking the CMake version (this problem: https://discourse.cmake.org/t/could-not-find-cmake-root/216/13). Either now you follow the first approach or you delete the old version and remove the old version (described at the top of [Installing the Latest CMake on Ubuntu Linux](https://graspingtech.com/upgrade-cmake/)), and reinstall. The catkin build command will not work anymore. Then reinstall everything starting from the top of this page.
  ...
  ```bash  
   sudo apt-get install cmake
  ``` 
  but it will not necessarily install the newest CMake version.
...
### shell_additions
Contains __shell_additions.sh__ : which contains the definitions of used shell functions used in the testing_brubotics package.
...
* Create a symbolic link to a directory as explained [here](https://www.freecodecamp.org/news/symlink-tutorial-in-linux-how-to-create-and-remove-a-symbolic-link/) and [here](https://linuxize.com/post/how-to-create-symbolic-links-in-linux-using-the-ln-command/).  
